    ...
    protected void doFilterInternal(HttpServletRequest httpServletRequest, HttpServletResponse httpServletResponse,
        ...
        try {
            Jws<Claims> claimsJws = Jwts.parser()
                    .setSigningKey(keySecretExample.getBytes())
                    .parseClaimsJws(token);
            Claims body = claimsJws.getBody();
            String userName = body.getSubject();
            var authentication = (Set<Map<String, String>>) body.get("Authentication");

            Set<SimpleGrantedAuthority> authority = authentication.stream()
            ...
            SecurityContextHolder.getContext().setAuthentication(authorization); // to authenticate the user
        }catch (JwtException e ){
            throw new IllegalArgumentException("token not be trusted " + token);
        }
        }
}
